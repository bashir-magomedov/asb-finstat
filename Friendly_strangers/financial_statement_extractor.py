...
class Locator:
    ...
    def _heuristic(pages,terms):
        scores=[(sum(p.lower().count(t) for t in terms),n) for n,p in enumerate(pages,1)]
        score,page=max(scores,default=(0,0)); return [page] if score else []
...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--input-file",type=Path); p.add_argument("--input-dir",type=Path,default=ROOT/"inputs"); p.add_argument("--output-dir",type=Path,default=ROOT/"outputs"); p.add_argument("--model",default="google/gemini-2.5-flash"); p.add_argument("--ocr-mode",choices=("auto","off","required"),default="auto"); p.add_argument("--no-openrouter",action="store_true"); a=p.parse_args()
