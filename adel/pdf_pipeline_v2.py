...
MIN_LANG_CONFIDENCE = 0.50
DEFAULT_OCR_LANGS = "eng+ara+fra+deu+spa+chi_sim"
# GoogleTranslator uses URL-encoded GET requests. Non-Latin scripts (Arabic,
# CJK, Cyrillic, Hebrew, Thai …) encode to 2-3× more bytes than characters,
# so we budget by UTF-8 byte length rather than character count. 500 bytes is
# safe for every script while all chunks are rejoined in their original order.
...
def detect_language(
    ...
    sample = unicodedata.normalize("NFKC", text[:sample_size]).strip()
    if not sample:
        return "unknown", None
    try:
        results = detect_langs(sample)
    except LangDetectException as exc:
        logger.debug("langdetect could not detect language (no features): %s", exc)
        return "unknown", None
    if not results:
        return "unknown", None
    ...
    if confidence < min_confidence:
        logger.debug(
            "Language confidence %.2f is below threshold %.2f; treating as unknown.",
...
def _translation_source_language(page_language: dict[str, object]) -> Optional[str]:
    """Choose a non-English source language from a page's composition."""
    primary_code = str(page_language["language_code"])
    if primary_code not in {"en", "unknown"}:
        return primary_code

    for language in page_language["languages"]:
...
def translate_pages_to_english(
    ...
    for index, page_text in enumerate(page_texts, start=1):
        ...
        translation_source = _translation_source_language(language)

        if language_code == "unknown" and translation_source is None:
            logger.warning(
                "Page %d - translation skipped: detected language is unknown.",
                index,
            )
        elif translation_source is None:
            logger.debug("Page %d - English text kept as-is.", index)
        else:
            if language_code == "en":
                logger.info(
                    "Page %d - English is primary, but %s (%s) is also present; translating.",
                    index,
                    resolve_language_name(translation_source),
                    translation_source,
                )
            else:
                logger.info(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Translate the complete extracted document to English with Google Translate.",
    )
    parser.add_argument(
        "--translate-batch-size",
        type=int,
        default=DEFAULT_TRANSLATE_BATCH_SIZE,
        help="Maximum characters sent to Google Translate in a single request.",
    )
    parser.add_argument(
        "--log-level",
